...
const orbits = [];
const labels = []; 
// Array to hold label elements
sunGeometry = new THREE.SphereGeometry(2.5 * sizeMultiplier, 32, 32);
...
function initializeSun() {
  ...
  sunLabelDiv.textContent = sun.userData.name;
  sunLabelDiv.style.color = "lightblue";
  ...
  sunLabelDiv.className = "label";
  sunLabelDiv.textContent = sunData.name;
  sunLabelDiv.style.color = "lightyellow";
  sunLabelDiv.style.position = "absolute";
  sunLabelDiv.style.fontFamily = "Michroma";
  sunLabelDiv.style.fontSize = "16px";
  sunLabelDiv.style.pointerEvents = "none";
  sunLabelDiv.style.whiteSpace = "nowrap";
  sunLabelDiv.style.textShadow = "1px 1px 0 black,-1px -1px 0 black";
  scene.add(sun);
  labels.push({ planet: sun, labelDiv: sunLabelDiv });
  /*
  justTheSun.forEach((data) => {
    sun.userData=data;
    ...
    labels.push({ sun: sun, labelDiv: labelDiv });
...
function sunPopup(data) {
  ...
  const periodPara = document.createElement("p");
  periodPara.innerHTML = `<strong>Orbital Period:</strong> ${data.period} Years`;
  popup.appendChild(periodPara);

  const axisPara = document.createElement("p");
  axisPara.innerHTML = `<strong>Semi-Major Axis:</strong> ${data.rotate} days`;
  popup.appendChild(axisPara);
